...
export default function FormularioDeAvaImersaoEmMindfulnessPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      toggleLoader(true);
      try {
        const formsRes= await  api.get<IDataTableResposne<SubmissionRecord[]>>("/forms/formulario_de_ava_imersao_em_mindfulness/submissions", {
            params: {
  ...
  return (
    <Section title="Formulário de Ava Imersão em Mindfulness">
      <div className="space-y-8">
      </div>
      <div className="overflow-x-auto">
        <DataTable<SubmissionRecord, unknown>
          data={forms}
          columns={getColumns('/fichas/formulario_de_ava_imersao_em_mindfulness')}
      ...
      <div className="flex w-full justify-end">
        <Button
          onClick={() => router.push('/forms/formulario_de_ava_imersao_em_mindfulness')}
          type="button"
          variant="outline"
